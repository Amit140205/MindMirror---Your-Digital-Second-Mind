...
import { chatAPI } from "../../shared/api/api.js"
...
export default function ChatArea({ isTutorialOpen, tutorialSlide }) {
  ...
  const isTutorialMessages = isTutorialOpen && tutorialSlide === 2 && messages.length === 0
  const renderMessages = isTutorialMessages 
    ? [
        { role: "user", text: "What was I searching yesterday about React?" }, 
        { role: "ai", data: { 
            answer: "Yesterday, you spent 14 minutes reading about React state management...", 
            sources: [], 
            follow_up_questions: [] 
          } 
        }
    ...
    <div style={{
      ...
      <div style={{
        ...
        {!showMessages ? (
          ...
          <div 
            className={isTutorialOpen && tutorialSlide === 2 ? "tutorial-highlight" : ""}
            style={{ 
              width: "100%", 
              maxWidth: "800px", 
              display: "flex", 
                    ...
                    <div style={{
                      ...
                      {msg.data.sources && msg.data.sources.length > 0 && (
                        <div style={{ marginTop: "16px" }}>
                          <p style={{ fontSize: "11px", fontWeight: 700, color: "var(--text-disabled)", textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: "8px" }}>
                            Sources
                          </p>
                          <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
                            {msg.data.sources.map((source, idx) => (
                              ...
                              >
                                ...
                                <span style={{ color: "var(--text-disabled)", fontSize: "10px", marginTop: "2px" }}>
                          ...
                          <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
                            {msg.data.follow_up_questions.map((q, idx) => (
                              <button
                                ...
                                onMouseEnter={e => !isLoading && (e.currentTarget.style.background = "var(--primary-glow)")}
      ...
      <div style={{
        ...
        <div 
          className={isTutorialOpen && tutorialSlide === 0 ? "tutorial-highlight" : ""}
          style={{
            width: "100%",
            maxWidth: "800px",
            ...
            alignItems: "center",
            gap: "12px",
            boxShadow: isLoading ? "none" : "0 8px 32px rgba(108, 99, 255, 0.15)",
        ...
        >
          ...
          <button 
            ...
            style={{
              background: isLoading || !inputText.trim() ? "var(--border)" : "var(--primary)",
              border: "none",
              borderRadius: "16px",
              width: "40px",
              height: "40px",
              ...
              color: isLoading || !inputText.trim() ? "var(--text-disabled)" : "#fff",
              transition: "transform 0.1s ease, background 0.2s ease",
            }}
            onMouseEnter={e => {
              if (!isLoading && inputText.trim()) e.currentTarget.style.backgroundColor = "var(--primary-hover)"
            }}
            onMouseLeave={e => {
              if (!isLoading && inputText.trim()) e.currentTarget.style.backgroundColor = "var(--primary)"
            }}
            onMouseDown={e => {
              if (!isLoading && inputText.trim()) e.currentTarget.style.transform = "scale(0.95)"
            }}
            onMouseUp={e => {
              if (!isLoading && inputText.trim()) e.currentTarget.style.transform = "scale(1)"
            }}
          >
